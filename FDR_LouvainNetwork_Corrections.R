...
## This script is organized differently than the Shen scripts. Instead of correcting across Networks, it is corrected across metrics

## So for Low, Medium,  High are sorted into different scripts. For each Louvain network (low, medium, or high)  it pulls the pvalues for the various  metrics performs an FDR Correction on them
## Then it saves them into an excel based on the four dimensions. 
## NOTE This script labels the metrics in this order: ASP, SWS, Diameter, Local efficiency, Modularity, and Small world omega - so if your MPlus script labels the networks in this order, then it will be consistent, if not then you will have to change the MPlus script or this script to reflect the order of your networks 

## Run this script to save the function. When it runs, FDR_correct_louvain(output) will be the function name/ syntax.
## 'output' should be replaced with the MPlus out file name without the .out (ex. if the MPlus script is called  Mid10_Low_All.out, output should be set to "Mid10_Low_all" WITH quotation marks)


FDR_correct_louvain <- function(output) {
